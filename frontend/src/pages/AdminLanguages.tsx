...
import { Globe, Plus, Edit2, Trash2, Search, AlertCircle, CheckCircle, RefreshCw, X } from 'lucide-react';
...
export default function AdminLanguages() {
  ...
  const [deletingLang, setDeletingLang] = useState<LanguageItem | null>(null);
  ...
  const fetchLanguages = async () => {
    ...
    try {
      ...
      showError('Failed to load language directory');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddLanguage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post('/languages', { name: nameInput.trim() });
      const msg = res.data.message || `Language "${nameInput.trim()}" added successfully!`;
      setSuccessMsg(msg);
      showSuccess(msg);
      setNameInput('');
      fetchLanguages();
      ...
      const errMsg = err.response?.data?.message || 'Failed to create language';
      setErrorMsg(errMsg);
      showError(errMsg);
    } finally {
      setSubmitting(false);
  ...
  const handleEditLanguage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingLang || !nameInput.trim()) return;
    ...
    try {
      const res = await api.put(`/languages/${editingLang._id}`, { name: nameInput.trim() });
      const msg = res.data.message || 'Language updated successfully!';
      setSuccessMsg(msg);
      showSuccess(msg);
      fetchLanguages();
      setTimeout(() => {
      ...
      const errMsg = err.response?.data?.message || 'Failed to update language';
      setErrorMsg(errMsg);
      showError(errMsg);
    } finally {
      setSubmitting(false);
  ...
  const executeDeleteLanguage = async () => {
    ...
    try {
      const res = await api.delete(`/languages/${deletingLang._id}`);
      showSuccess(res.data.message || `Language "${deletingLang.name}" deleted successfully`);
      fetchLanguages();
    } catch (err: any) {
      showError(err.response?.data?.message || 'Failed to delete language');
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-paper-300 dark:border-charcoal-300">
        <div>
          <h1 className="text-3xl font-serif font-bold text-ink dark:text-paper-100 flex items-center gap-2">
            <Globe className="w-7 h-7 text-primary" /> Language Directory
          </h1>
          <p className="text-sm text-ink-muted dark:text-paper-400 mt-1">
            Manage book languages (e.g. English, Hindi, Gujarati). Pre-configured so admins can pick cleanly when adding books.
          </p>
        </div>
        ...
        >
          <Plus className="w-4 h-4" /> Add New Language
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-white dark:bg-charcoal-100 rounded-xl border border-paper-300 dark:border-charcoal-border space-y-3">
          <Globe className="w-12 h-12 mx-auto text-ink-muted opacity-50" />
          <h3 className="font-serif font-bold text-lg text-ink dark:text-paper-100">No Languages Found</h3>
          <p className="text-sm text-ink-muted dark:text-paper-400">
            {search ? 'No languages match your search.' : 'Add languages to configure your catalog.'}
          </p>
        </div>
      ) : (
          ...
          <div className="max-w-md w-full bg-white dark:bg-charcoal-100 rounded-2xl shadow-card p-6 border border-paper-300 dark:border-charcoal-50 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-paper-200 dark:border-charcoal-300">
              <h3 className="font-serif font-bold text-xl text-ink dark:text-paper-100 flex items-center gap-2">
                <Plus className="w-5 h-5 text-primary" /> Add New Language
              </h3>
              <button
                onClick={() => {
                  setShowAddModal(false);
            ...
            <form onSubmit={handleAddLanguage} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Sanskrit, French, Marathi"
                  value={nameInput}
                  onChange={(e) => {
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {submitting ? 'Saving...' : 'Add Language'}
                </button>
              </div>
            </form>
          </div>
      ...
      {showEditModal && editingLang && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-white dark:bg-charcoal-100 rounded-2xl shadow-card p-6 border border-paper-300 dark:border-charcoal-50 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-paper-200 dark:border-charcoal-300">
              <h3 className="font-serif font-bold text-xl text-ink dark:text-paper-100 flex items-center gap-2">
                <Edit2 className="w-5 h-5 text-primary" /> Edit Language
              </h3>
              <button
                onClick={() => {
                  setShowEditModal(false);
      ...
      <ConfirmModal
        ...
        title="Delete Language"
        message={`Are you sure you want to delete language "${deletingLang?.name}"? Books associated with this language will remain in inventory.`}
        confirmText="Delete Language"
        cancelText="Cancel"
        type="danger"
        onConfirm={executeDeleteLanguage}
